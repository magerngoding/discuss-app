...
class CProfile extends ChangeNotifier {
  ...
  setFollow(BuildContext context, fromIdUser, String toIdUser) {
    if (isFollowing) {
      FollowSource.noFollowing(fromIdUser, toIdUser).then((success) {
        if (success) {
          ...
          DInfo.snackBarSuccess(context, 'Unfollow Failed!');
        }
      });
    } else {
      FollowSource.following(fromIdUser, toIdUser).then(
        (success) {
          if (success) {
            setStat(toIdUser);
            checkIsFollowing(fromIdUser, toIdUser);
            ...
            DInfo.snackBarSuccess(context, 'follow Failed!');
          }
        },
      );
    }
